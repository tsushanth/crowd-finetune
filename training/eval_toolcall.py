...
from backend import config, db, gold, reward, toolcall  # noqa: E402

COMPETITOR_PRICES = {
    "openai/gpt-4o-mini": (0.15, 0.60),
    "anthropic/claude-haiku-4.5": (1.00, 5.00),
}
DEFAULT_COMPETITORS = ["openai/gpt-4o-mini", "anthropic/claude-haiku-4.5"]
COMPETITOR_ALIASES = {
...
def main() -> None:
    ...
    def api_generator(model_id: str, stats: dict):
        def generate(row: dict, prompt: list[dict]) -> str:
            ...
            answer = gold.chat(toolcall.SYSTEM_PROMPT, row["question"], model=model_id).strip()
    ...
    def score(generate, label: str) -> float:
        ...
        for row, prompt in zip(rows, prompts):
            ...
            res = toolcall.is_correct(answer, row)
            correct += int(res["correct"])
            print(f"{int(res['correct'])}/1\t{row['corpus_id']}\t{label}\t{res['reason']}")
    ...
    print()
    print(f"{'model':<42}{'acc':>8}{'$/1K q':>10}{'tok/q':>8}{'tps':>10}")
